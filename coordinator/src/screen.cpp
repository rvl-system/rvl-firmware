...
void Screen::updatePreset(byte preset) {
  ...
  for (int i = 0; i < 3; i++) {
    if (presetValueLabels[preset][i] != NULL) {
      ...
      display.setColor(PIXEL_OFF);
      // + 2 is a hack to clear descenders
...
void Screen::updateValue(byte preset, byte code, byte value) {
  drawHorizontalBar(VALUE_BAR_X, VALUEX_Y[code], VALUE_BAR_WIDTH, TEXT_HEIGHT, ((double)value - presetValueMin[preset][code]) / ((double)presetValueMax[preset][code] - presetValueMin[preset][code]));
  display.display();
}
