...
const BRIGHTNESS_HIGHLIGHT_HEIGHT =
  BRIGHTNESS_BAR_HEIGHT + BRIGHTNESS_ICON_SIZE + BAR_SEPARATION + HIGHIGHT_PADDING * 2;
...
function drawControlOutline(control: number, color: number) {
  runOperationByControl(control, {
    Brightness: () => oled.drawRect(
      BRIGHTNESS_HIGHLIGHT_X,
      BRIGHTNESS_HIGHLIGHT_Y,
      BRIGHTNESS_HIGHLIGHT_WIDTH - 1,
      BRIGHTNESS_HIGHLIGHT_HEIGHT,
      color,
      false),
    Preset: () => oled.drawRect(
      PRESET_HIGHLIGHT_X,
      PRESET_HIGHLIGHT_Y,
      PRESET_HIGHLIGHT_WIDTH - 1,
      PRESET_HIGHLIGHT_HEIGHT,
      color,
      false),
    Value1: () => oled.drawRect(
      VALUE_HIGHLIGHT_X,
      VALUE1_HIGHLIGHT_Y,
      VALUE_HIGHLIGHT_WIDTH - 1,
      VALUE_HIGHLIGHT_HEIGHT,
      color,
      false),
    Value2: () => oled.drawRect(
      VALUE_HIGHLIGHT_X,
      VALUE2_HIGHLIGHT_Y,
      VALUE_HIGHLIGHT_WIDTH - 1,
      VALUE_HIGHLIGHT_HEIGHT,
...
function updateBrightness(brightness: number) {
  drawVerticalBar(
    BRIGHTNESS_BAR_X,
    BRIGHTNESS_BAR_Y,
    BRIGHTNESS_BAR_WIDTH,
    BRIGHTNESS_BAR_HEIGHT,
...
function updatePreset(preset: number) {
  ...
  runOperationByPreset(preset, {
    ...
    Pulse: () => {
      ...
      drawHorizontalBar(
        VALUE_BAR_X, VALUE3_Y, VALUE_BAR_WIDTH, TEXT_HEIGHT, settings.pulseValues.saturation / MAX_VALUE);
    }
  });
  oled.update();
}
